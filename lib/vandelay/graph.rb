  ...
  class Graph
    ...
    def dependencies_of(id)
      TSort.each_strongly_connected_component_from(id, ->(n, &b) {
        fetch_node(n, :source).incoming.each(&b)
      })
    end
    ...
    def dependants_of(id)
      TSort.each_strongly_connected_component_from(id, ->(n, &b) {
        fetch_node(n, :target).outgoing.each(&b)
      })
    end
    ...
    sig {params(source_id: String, target_id: String, block: T.proc.params(arg0: ResourceInfo, arg1: ResourceInfo).void).void}
    def with_source_and_target(source_id, target_id, &block)
      yield(
        fetch_node(source_id, :source),
        fetch_node(target_id, :target)
      )
    end
  end
end
